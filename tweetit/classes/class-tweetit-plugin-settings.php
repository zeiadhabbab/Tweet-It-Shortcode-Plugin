...
/**
 * Tweetit_Plugin_Settings Class
 *
 * @class Tweetit_Plugin_Settings
 * @version	1.0.0
 * @since 1.0.0
 * @package	Tweetit_Plugin
 * @author Jeffikus
 */
final class Tweetit_Plugin_Settings {
	/**
	 * Tweetit_Plugin_Admin The single instance of Tweetit_Plugin_Admin.
	 * @var 	object
	 * @access  private
	 * @since 	1.0.0
	 */
	...
	/**
	 * Main Tweetit_Plugin_Settings Instance
	 *
	 * Ensures only one instance of Tweetit_Plugin_Settings is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Main Tweetit_Plugin_Settings instance
	 */
		...
		if ( is_array( $input ) && 0 < count( $input ) ) {
			...
			foreach ( $input as $k => $v ) {
				...
				if ( ! method_exists( $this, $method ) ) {
					if ( true === (bool)apply_filters( 'tweetit-plugin-validate-field-' . $fields[$k]['type'] . '_use_default', true ) ) {
				...
				if ( '' != $method ) {
					add_filter( 'tweetit-plugin-validate-field-' . $fields[$k]['type'], array( $this, $method ) );
				}

				$method_output = apply_filters( 'tweetit-plugin-validate-field-' . $fields[$k]['type'], $v, $fields[$k] );
	...
	public function render_field ( $args ) {
		...
		// Construct the key.
		$key 				= Tweetit_Plugin()->token . '-' . $args['section'] . '[' . $args['id'] . ']';
		...
		if ( isset( $args['type'] ) && ! in_array( $args['type'], (array)apply_filters( 'tweetit-plugin-no-description-fields', array( 'checkbox' ) ) ) ) {
			if ( isset( $args['description'] ) ) {
				$description = '<p class="description">' . wp_kses_post( $args['description'] ) . '</p>' . "\n";
				if ( in_array( $args['type'], (array)apply_filters( 'tweetit-plugin-new-line-description-fields', array( 'textarea', 'select' ) ) ) ) {
	...
	public function get_settings_sections () {
		...
		$settings_sections['standard-fields'] = __( 'Standard Fields', 'tweetit-plugin' );
		...
		return (array)apply_filters( 'tweetit-plugin-settings-sections', $settings_sections );
	...
	public function get_settings_fields ( $section ) {
		...
		switch ( $section ) {
			case 'standard-fields':
					...
					$settings_fields['twitter_account'] = array(
							'name' => __( 'Twitter Username', 'tweetit' ),
							'type' => 'text',
							'default' => '',
							'section' => 'standard-fields',
							'description' => __( 'Using your twitter username results in via @username.', 'tweetit-plugin' )
					);
					
					$settings_fields['facebook_app_id'] = array(
							'name' => __( 'Facebook App', 'tweetit' ),
							'type' => 'text',
							'default' => '',
							'section' => 'standard-fields',
							'description' => __( 'Using your facebook app ID , you can get one from here <a href="https://developers.facebook.com/" target="_blank">facebook developer</a>.', 'tweetit-plugin' )
					);
											
					$settings_fields['show_facebook'] = array(
							'name' => __( 'Show Facebook Icon', 'tweetit' ),
							'type' => 'checkbox',
							'default' => '',
							'section' => 'standard-fields',
							'description' => __( '', 'tweetit-plugin' )
					);											
											
					$settings_fields['show_twitter'] = array(
							'name' => __( 'Show Twitter Icon', 'tweetit' ),
							'type' => 'checkbox',
							'default' => '',
							'section' => 'standard-fields',
							'description' => __( '', 'tweetit-plugin' )
					...
					$settings_fields['back_color'] = array(
							'name' => __( 'Background Color', 'tweetit' ),
							'type' => 'radio',
							'default' => 'true',
							'section' => 'standard-fields',
		...
		return (array)apply_filters( 'tweetit-plugin-settings-fields', $settings_fields );
	...
	public function get_supported_fields () {
		return (array)apply_filters( 'tweetit-plugin-supported-fields', array( 'text', 'checkbox', 'radio', 'textarea', 'select', 'select_taxonomy' ) );
	...
	public function get_value ( $key, $default, $section ) {
		$values = get_option( 'tweetit-plugin-' . $section, array() );
	...
	public function get_settings ( $section = '' ) {
		...
		if ( 0 < count( $sections ) ) {
			foreach ( $sections as $k => $v ) {
				...
				$values = get_option( 'tweetit-plugin-' . $v, array() );
